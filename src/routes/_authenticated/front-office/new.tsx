...
export const Route = createFileRoute("/_authenticated/front-office/new")({
  component: () => (
...
function FrontOfficePage() {
  ...
  // Cached raw source rows for instant re-filtering on shift dropdown changes without re-fetching
  const [sourceDataCache, setSourceDataCache] = React.useState<{
  ...
  const recordAppointmentHistory = async (rows: FrontOfficeRow[], sourceLabel: string) => {
    const response = await fetch("/api/front-office/appointment-history/sync", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        sourceLabel,
        appointments: rows.map((row) => ({
          ...
          serviceName: row.source === "consultation" ? row.purpose : row.procedure,
          schedule: row.source === "consultation" ? row.schedule : "",
          invoiceNo: row.invoiceNo || "",
          billAmount: row.billAmount || 0,
          collectedAmount: row.collected || 0,
    ...
    if (!response.ok) {
      ...
      throw new Error(data.error || "Could not record appointment history");
    }
    const result = await response.json() as { success: boolean; recorded: number };
    queryClient.invalidateQueries({ queryKey: ["front-office", "patient-appointment-history"] });
  ...
  const processNormalizedSourceRows = async (
    consultationRaw: Record<string, any>[],
    procedureRaw: Record<string, any>[],
    radiologyRaw: Record<string, any>[],
    reportDateStr: string,
    sourceLabel: string,
    shiftOptionOverride?: string
  ) => {
    // Cache raw source data for instant re-filtering upon shift selector changes
    setSourceDataCache({
      consultationRaw,
      procedureRaw,
      radiologyRaw,
    ...
    // Appointment history is persisted from the complete source dataset before
    // applying a shift filter, so it never depends on Razorpay or the selected view.
    await recordAppointmentHistory([...consultations, ...procedures, ...radiology], sourceLabel);

    const totalRawCount = consultations.length + procedures.length + radiology.length;
    const effectiveShiftOption = shiftOptionOverride !== undefined ? shiftOptionOverride : selectedShiftOption;

    // Determine shift filter to apply
    let shiftToApply: FrontOfficeShift | null = null;
    let effectiveShiftName = "Full Day";

    if (effectiveShiftOption !== "all") {
      if (effectiveShiftOption === "auto") {
        const now = new Date();
        // 1. Live matching: Check if current live time falls within any active user-configured shift
        let matched = activeShifts.find((s) => isTimeInShiftWindow(now, s.startTime, s.endTime));

        // 2. Data timestamp matching: If current time is outside shift windows (e.g. reviewing off-hours, or importing past shift),
        // inspect transaction timestamps in the dataset and auto-select the user-configured shift that contains the most transactions!
        if (!matched && (consultations.length > 0 || procedures.length > 0 || radiology.length > 0)) {
          const allTimestamps = [...consultations, ...procedures, ...radiology]
            .map((r) => r.dateObj || parseDate(r.dateText))
            .filter((d): d is Date => Boolean(d && !isNaN(d.getTime())));

          if (allTimestamps.length > 0) {
            let maxCount = 0;
            let bestShift: FrontOfficeShift | null = null;
            for (const s of activeShifts) {
              const count = allTimestamps.filter((t) => isTimeInShiftWindow(t, s.startTime, s.endTime)).length;
              if (count > maxCount) {
                maxCount = count;
                bestShift = s;
              }
            }
            if (bestShift && maxCount > 0) {
              matched = bestShift;
            }
          }
        }

        // 3. Fallback: If no shift matched (no transactions or no overlap), fallback to Full Day instead of arbitrarily restricting to shift 1!
        shiftToApply = matched || null;
      } else {
        shiftToApply = activeShifts.find((s) => String(s.id) === effectiveShiftOption) || null;
      }
    }
  ...
  // Handler for changing shift selection: updates state and instantly re-filters cached source data
  const handleShiftOptionChange = (newOption: string) => {
    setSelectedShiftOption(newOption);
    if (sourceDataCache) {
      try {
        void processNormalizedSourceRows(
          sourceDataCache.consultationRaw,
          sourceDataCache.procedureRaw,
          sourceDataCache.radiologyRaw,
          sourceDataCache.reportDateStr,
          sourceDataCache.sourceLabel,
          newOption
        ).catch((err: any) => toast.error(err.message || "Failed to re-filter transactions for selected shift"));
      } catch (err: any) {
        toast.error(err.message || "Failed to re-filter transactions for selected shift");
      }
    } else {
      const matched = activeShifts.find((s) => String(s.id) === newOption);
      if (matched) {
      ...
      } else if (newOption === "auto") {
        const now = new Date();
        const liveMatch = activeShifts.find((s) => isTimeInShiftWindow(now, s.startTime, s.endTime));
        if (liveMatch) {
          setActiveShiftLabel(`${liveMatch.name} (${liveMatch.startTime} – ${liveMatch.endTime})`);
        } else {
          setActiveShiftLabel("Full Day (All Hours)");
        }
      }
  ...
  const handleProcessFiles = async () => {
    ...
    try {
      ...
      await processNormalizedSourceRows(
        consultationRaw,
        procedureRaw,
        radiologyRaw,
  ...
  const [showAllSystemRemarks, setShowAllSystemRemarks] = React.useState(false);
  ...
  // State to track in-flight Docterz link resolution per patient
  const [loadingDocterzKey, setLoadingDocterzKey] = React.useState<string | null>(null);
  const [historyPatient, setHistoryPatient] = React.useState<CompiledPatient | null>(null);
  const appointmentHistoryQuery = useQuery<{ data: PatientHistoryRow[]; total: number }>({
    queryKey: ["front-office", "patient-appointment-history", historyPatient?.patientUid, historyPatient?.patientName],
    enabled: Boolean(historyPatient),
    queryFn: async () => {
      const params = new URLSearchParams();
      if (historyPatient?.patientUid) params.set("uid", historyPatient.patientUid);
      if (historyPatient?.patientName) params.set("name", historyPatient.patientName);
      ...
      if (!response.ok) throw new Error("Could not load appointment history");
      return response.json();
    },
  });

  const handleOpenDocterz = async (patient: CompiledPatient) => {
    const key = `${patient.patientUid || patient.patientName}`;
    setLoadingDocterzKey(key);
    const toastId = toast.loading(`Resolving Docterz for ${patient.patientName || "patient"}...`);

    // Synchronously open window before asynchronous network fetch to prevent browser popup blockers
    ...
    try {
      const res = await fetch("/api/front-office/resolve-patient-url", {
        ...
        body: JSON.stringify({
          ...
          invoiceNo: patient.invoiceNos && patient.invoiceNos[0],
        }),
      });

      if (!res.ok) {
        throw new Error(`HTTP ${res.status}: Failed to resolve patient deep link`);
      }

      const data = await res.json();
      if (!data.success) {
        if (newWindow && !newWindow.closed) newWindow.close();
        toast.error(data.message || `Failed to locate ${patient.patientName} in Docterz`, { id: toastId });
        return;
      }

      const url = data.recordsUrl || data.invoicesUrl;
      if (!url) {
        if (newWindow && !newWindow.closed) newWindow.close();
        toast.error("Docterz URL not available", { id: toastId });
        return;
      }

      toast.success("Opening Docterz in new tab...", { id: toastId });
      ...
      if (newWindow && !newWindow.closed) newWindow.close();
      toast.error(err.message || "Failed to resolve patient in Docterz", { id: toastId });
    } finally {
      setLoadingDocterzKey(null);
    }
  };
    ...
    if (searchQuery.trim()) {
      ...
      list = compiledPatients.filter((p) => {
        const searchStr = [
          ...
          ...(p.invoiceNos || []),
          ...(p.apiRemarks || []),
          ...p.consultations.flatMap((c) => [c.doctor, c.purpose, c.schedule, c.paymentMode, c.dateText, c.notes || "", c.invoiceNo || ""]),
          ...p.procedures.flatMap((pr) => [pr.procedure, pr.doctor, pr.paymentMode, pr.category, pr.dateText, pr.notes || "", pr.invoiceNo || ""]),
        ]
          .join(" ")
          .toLowerCase();
  ...
  const visitsByTimeSlotData = React.useMemo<ChartCountItem[]>(() => {
    const parseTimeSlotStartHour = (label: string): number => {
    ...
    if (consultationRows.length > 0) {
      ...
      return Object.entries(counts)
        .map(([name, value]) => ({ name, value }))
        .sort((a, b) => parseTimeSlotStartHour(a.name) - parseTimeSlotStartHour(b.name));
    }
    if (archivedSummaryData?.visitsByTimeSlotData?.length) {
      return [...archivedSummaryData.visitsByTimeSlotData].sort(
        (a, b) => parseTimeSlotStartHour(a.name) - parseTimeSlotStartHour(b.name)
      );
    }
    return [];
  }, [consultationRows, archivedSummaryData]);
  ...
  const handleLoadSavedReport = async (reportId: number) => {
    try {
      ...
      try {
        await recordAppointmentHistory(restoredAllRows, `Archived Front Office report #${reportId}`);
      } catch (historyError) {
        console.error("Could not backfill appointment history from archived report", historyError);
        toast.warning("The report loaded, but its appointment history could not be recorded.");
      }
  ...
  const initialReportLoadedRef = React.useRef(false);
  React.useEffect(() => {
    ...
    void handleLoadSavedReport(reportId);
  }, []);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="relative overflow-hidden rounded-2xl bg-linear-to-r from-emerald-900 via-teal-800 to-green-800 text-white shadow-xl p-6 sm:p-8">
        <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-6 relative z-10">
          ...
          <div className="flex flex-wrap items-center gap-2 self-start lg:self-center bg-black/20 p-1.5 rounded-xl backdrop-blur-md border border-white/10">
            ...
            <Link
              to={"/front-office/razorpay-reconciliation" as any}
              className="px-4 py-2 rounded-lg text-xs font-bold transition-colors cursor-pointer flex items-center gap-1.5 text-white/80 hover:text-white"
              title="Reconcile Razorpay payments with patient records"
            >
              <CreditCard className="size-3.5" />
              Razorpay Reconciliation
            </Link>
            <Link
              to="/front-office/patients"
              className="px-4 py-2 rounded-lg text-xs font-bold transition-colors cursor-pointer flex items-center gap-1.5 text-white/80 hover:text-white"
              title="Open Front Office Patient Directory"
            >
            ...
            <div className="flex flex-wrap items-center gap-3">
                ...
                <Popover>
                  <PopoverTrigger asChild>
                    <Button
                      variant="outline"
                      size="sm"
                      className="h-8 text-xs font-semibold justify-start text-left px-2.5 gap-2 min-w-32 bg-background hover:bg-muted/50 cursor-pointer"
                    >
                      <span>
                        {activeReportDate
              ...
              <div className="flex items-center gap-2">
                ...
                <select
                  ...
                  className="h-8 text-xs font-semibold rounded-md border bg-background px-2.5 py-1 text-foreground cursor-pointer"
                >
                ...
                <table className="w-full text-xs">
                  <thead>
                    <tr className="bg-muted/70 border-b text-slate-700 dark:text-slate-200 font-bold uppercase text-[11px]">
                      ...
                      <th className="py-2 px-3 text-left min-w-50">PATIENT NAME</th>
                      <th className="py-2 px-3 text-right w-36">AMOUNT (₹)</th>
                      <th className="py-2 px-3 text-left min-w-55">REMARK</th>
        ...
        <div className="space-y-6">
          ...
          {!hasData ? (
            ...
            <div id="frontOfficeReportContent" className="space-y-6">
              ...
              <Card className="shadow-sm border-blue-500/30 bg-linear-to-r from-blue-500/5 via-teal-500/5 to-emerald-500/5 overflow-hidden">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                ...
                <Card className="shadow-xs">
                  <CardHeader className="pb-2 flex flex-row items-center justify-between">
                    ...
                    {collectionsByPaymentModeData.length > 0 && (
                      <span className="text-xs font-bold text-emerald-600 dark:text-emerald-400">
                        {formatMoney(collectionsByPaymentModeData.reduce((s, i) => s + i.value, 0))}
                      </span>
                    )}
                  </CardHeader>
                  <CardContent className="h-64 overflow-y-auto">
                    {collectionsByPaymentModeData.length === 0 ? (
                      ...
                      <table className="w-full text-xs">
                        <thead>
                          <tr className="border-b text-left sticky top-0 bg-background/95 backdrop-blur-xs">
                            <th className="py-2 px-2.5 font-bold">Payment Mode</th>
                            <th className="py-2 px-2.5 font-bold text-center w-28">Share</th>
                            <th className="py-2 px-2.5 font-bold text-right">Amount Collected</th>
                          </tr>
                        </thead>
                        <tbody className="divide-y">
                          {(() => {
                            const total = collectionsByPaymentModeData.reduce((s, item) => s + item.value, 0);
                            return collectionsByPaymentModeData.map((item: ChartCountItem, i: number) => {
                              const pct = total > 0 ? (item.value / total) * 100 : 0;
                              const style = getPaymentModeStyle(item.name);
                              return (
                                <tr key={i} className="hover:bg-muted/40">
                                  <td className="py-2 px-2.5 font-medium">
                                    <div className="flex items-center gap-2">
                                      ...
                                      />
                                      <span className={`inline-flex items-center px-1.5 py-0.5 rounded text-[11px] font-semibold border ${style.badgeClass}`}>
                                        {item.name}
                                      </span>
                                    </div>
                                  </td>
                                  <td className="py-2 px-2.5 text-center">
                                    <div className="flex items-center gap-1.5 justify-center">
                                      ...
                                      <span className="text-[10.5px] text-muted-foreground font-mono w-9 text-right">
                                        {pct.toFixed(1)}%
                                      </span>
                                    </div>
                                  </td>
                        ...
                        <tfoot>
                          <tr className="border-t-2 font-bold bg-muted/60 sticky bottom-0">
              ...
              <Card className="shadow-sm">
                <CardHeader className="pb-3 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                  ...
                  <div className="flex items-center gap-2 w-full sm:w-auto">
                    <Button
                      variant={showAllSystemRemarks ? "secondary" : "outline"}
                      size="sm"
                      onClick={() => setShowAllSystemRemarks((prev) => !prev)}
                      className="text-xs h-9 gap-1.5 shrink-0"
                      title="Toggle showing automated system reconciliation notes inline"
                    >
                      <Info className="size-3.5 text-amber-600 dark:text-amber-400" />
                      <span>{showAllSystemRemarks ? "Hide System Commentary" : "Show System Commentary"}</span>
                      {compiledPatients.some((p) => (p.systemRemarks?.length || 0) > 0) && (
                        <Badge
                          variant="secondary"
                          className="px-1 py-0 text-[10px] h-4 ml-0.5 bg-amber-100 dark:bg-amber-950 text-amber-800 dark:text-amber-300 border-0"
                        >
                          {compiledPatients.filter((p) => (p.systemRemarks?.length || 0) > 0).length}
                        </Badge>
                      )}
                    </Button>
                    <div className="relative w-full sm:w-72">
                      ...
                      <Input
                        placeholder="Search patient, UID, doctor, test, remarks..."
                        value={searchQuery}
                    ...
                    <table className="w-full text-xs">
                      ...
                      <tbody className="divide-y">
                        {filteredPatients.map((p, idx) => {
                          const paymentModes = [
                            ...new Set(
                              [...p.consultations, ...p.procedures]
                                .flatMap((x) => (x.paymentMode || "").split(/,\s*/))
                          ...
                          const apiRemarks = [
                            ...new Set(
                              (p.apiRemarks || [])
                                .concat([...p.consultations, ...p.procedures].flatMap((x) => x.apiRemarks || (x.notes ? [x.notes] : [])))
                          ...
                          const systemRemarks = [
                            ...new Set(
                              (p.systemRemarks || [])
                                .concat([...p.consultations, ...p.procedures].flatMap((x) => x.remarks || []))
                                .filter((s): s is string => Boolean(s && s.trim()))
                            ),
                          ];
                            ...
                            <tr key={p.patientUid || idx} className="hover:bg-muted/30 transition-colors">
                              ...
                              <td className="py-3 px-3 font-bold text-foreground">
                                <div className="flex flex-col">
                                  <span>{p.patientName || "Unknown Patient"}</span>
                                  {p.mobile && (
                              ...
                              <td className="py-3 px-3 font-mono text-[11px]">
                                <div className="flex flex-col gap-1">
                                  <span className="text-muted-foreground">{p.patientUid || "No UID"}</span>
                                  <button
                                    type="button"
                                    onClick={() => handleOpenDocterz(p)}
                                    disabled={loadingDocterzKey === `${p.patientUid || p.patientName}`}
                                    className="inline-flex items-center gap-1 text-[9.5px] font-sans font-medium text-sky-700 dark:text-sky-300 hover:text-sky-800 dark:hover:text-sky-200 bg-sky-50 dark:bg-sky-950/60 hover:bg-sky-100 dark:hover:bg-sky-900/80 border border-sky-200 dark:border-sky-800 px-1.5 py-0.5 rounded cursor-pointer transition-colors w-fit"
                                    title="Open patient in Docterz app"
                                  >
                                    {loadingDocterzKey === `${p.patientUid || p.patientName}` ? (
                                  ...
                                  <button
                                    ...
                                    className="inline-flex items-center gap-1 text-[9.5px] font-sans font-medium text-violet-700 dark:text-violet-300 hover:text-violet-800 dark:hover:text-violet-200 bg-violet-50 dark:bg-violet-950/60 hover:bg-violet-100 dark:hover:bg-violet-900/80 border border-violet-200 dark:border-violet-800 px-1.5 py-0.5 rounded cursor-pointer transition-colors w-fit"
                                    title="View saved appointment and Razorpay reconciliation history"
                                  >
                              ...
                              <td className="py-3 px-3 space-y-1.5">
                                {p.consultations.length ? (
                                  p.consultations.map((c, i) => {
                                    const typeLabel = visitTypeLabel(c.purpose);
                                    const isFollowUp = /follow[\s-]?up/i.test(typeLabel);
                                    const isConsultation = /consultation/i.test(typeLabel);
                                    ...
                                    return (
                                      <div key={i} className="text-[11px] space-y-0.5">
                                        <div className="flex items-center gap-1.5 flex-wrap">
                                          <span className={`inline-block text-[10px] font-semibold py-0.5 px-1.5 rounded border shrink-0 ${badgeClass}`}>
                                            {typeLabel}
                                          </span>
                                          {c.invoiceNo && (
                                            <span
                                              className="inline-block text-[9.5px] font-mono font-medium px-1.5 py-0.5 rounded bg-amber-50 dark:bg-amber-950/40 text-amber-800 dark:text-amber-300 border border-amber-200 dark:border-amber-800/60 shrink-0"
                                              title={`Invoice #${c.invoiceNo}`}
                                            >
                                              Inv #{c.invoiceNo}
                                            </span>
                                          )}
                                ...
                                {paymentModes.length ? (
                                  <div className="flex flex-wrap gap-1">
                                    {paymentModes.map((m, i) => {
                                      const style = getPaymentModeStyle(m);
                                      return (
                                        <span
                                          key={i}
                                          className={`inline-flex items-center gap-1 text-[10px] font-semibold px-1.5 py-0.5 rounded border ${style.badgeClass}`}
                                        >
                                          <span
                              ...
                              <td className="py-3 px-3 max-w-64">
                                {apiRemarks.length ? (
                                  <ul className="list-disc pl-3 text-[11px] text-foreground space-y-0.5">
                                    {apiRemarks.map((rem, i) => (
                                    ...
                                    <Popover>
                                      <PopoverTrigger asChild>
                                        <Button
                                          variant="outline"
                                          size="sm"
                                          className="h-5 px-1.5 text-[10px] font-medium text-amber-700 hover:text-amber-800 dark:text-amber-300 dark:hover:text-amber-200 bg-amber-50 hover:bg-amber-100 dark:bg-amber-950/40 dark:hover:bg-amber-950/60 border-amber-300 dark:border-amber-800 gap-1 cursor-pointer transition-colors shadow-2xs"
                                          title="Click to view system reconciliation commentary for this patient"
                                        >
                                          <Info className="size-2.5 text-amber-600 dark:text-amber-400 shrink-0" />
                                          <span>Audit Notes ({systemRemarks.length})</span>
                                        </Button>
                                      </PopoverTrigger>
                                      <PopoverContent className="w-80 p-3 text-xs shadow-lg" align="end">
                                        <div className="flex items-center gap-1.5 font-bold text-foreground pb-1.5 border-b mb-2">
                                          <Info className="size-3.5 text-amber-600 dark:text-amber-400 shrink-0" />
                                          <span>System Audit Notes</span>
                                          <span className="text-[10.5px] font-normal text-muted-foreground ml-auto truncate max-w-36">
                                            {p.patientName}
                                          </span>
                                        </div>
                                        <p className="text-[10px] text-muted-foreground mb-2">
                                          Automated payment attribution and invoice reconciliation notes generated during processing:
                                        </p>
                                        <ul className="list-disc pl-3.5 space-y-1.5 text-[11px]">
                                          {systemRemarks.map((rem, i) => (
                                            <li
                                              key={i}
                                              className={
                                                rem.includes("⚠")
                                ...
                                {showAllSystemRemarks && systemRemarks.length > 0 && (
                                  <div className="mt-1.5 pt-1.5 border-t border-dashed border-amber-200 dark:border-amber-800/60">
                                    <span className="text-[9.5px] uppercase font-semibold tracking-wider text-amber-700 dark:text-amber-400 flex items-center gap-0.5 mb-0.5">
                                      <Info className="size-2.5" /> System Notes:
                                    </span>
      ...
      <Dialog open={Boolean(historyPatient)} onOpenChange={(open) => !open && setHistoryPatient(null)}>
        <DialogContent className="max-h-[85vh] overflow-hidden sm:max-w-5xl">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2">
              <History className="size-5 text-violet-600" />
              Appointment History — {historyPatient?.patientName || "Patient"}
            </DialogTitle>
            <DialogDescription>
              Front Office activity and linked Razorpay records{historyPatient?.patientUid ? ` for UID ${historyPatient.patientUid}` : " matched by patient name"}.
            </DialogDescription>
          </DialogHeader>
          <div className="max-h-[65vh] overflow-auto rounded-lg border">
            {appointmentHistoryQuery.isLoading ? (
              <div className="flex min-h-48 items-center justify-center gap-2 text-sm text-muted-foreground">
                <Loader2 className="size-4 animate-spin" /> Loading appointment history…
              </div>
            ) : appointmentHistoryQuery.isError ? (
              <div className="min-h-32 p-6 text-center text-sm text-destructive">Could not load appointment history.</div>
            ) : !appointmentHistoryQuery.data?.data.length ? (
              <div className="min-h-48 p-8 text-center">
                <History className="mx-auto mb-3 size-9 text-muted-foreground/50" />
                <p className="font-semibold">No saved appointments found</p>
                <p className="mt-1 text-xs text-muted-foreground">Fetch Docterz data or process Front Office CSV files to record this patient’s history.</p>
              </div>
            ) : (
              <table className="w-full min-w-200 text-left text-xs">
                <thead className="sticky top-0 bg-muted text-muted-foreground">
                  <tr><th className="p-3">Appointment</th><th className="p-3">Doctor / service</th><th className="p-3">Source record</th><th className="p-3 text-right">Amount</th><th className="p-3">Status</th><th className="p-3">Recorded from</th></tr>
                </thead>
                <tbody className="divide-y">
                  {consolidatePatientHistory(appointmentHistoryQuery.data.data).map((entry) => (
                    <tr key={entry.history_key} className="align-top hover:bg-muted/30">
                      <td className="p-3"><div className="font-semibold">{entry.appointment_id ? `#${entry.appointment_id}` : "No appointment ID"}</div><div className="mt-1 whitespace-nowrap text-muted-foreground" title={entry.appointment_date || entry.payment_date || undefined}>{formatPatientHistoryDate(entry.appointment_date || entry.payment_date)}</div></td>
                      ...
                      <td className="p-3">{entry.source_kind === "consolidated" ? <><div className="font-semibold text-emerald-700">API + Razorpay</div><div className="mt-1 font-mono text-muted-foreground">{entry.invoice_no ? `Inv ${entry.invoice_no}` : "No invoice"} · {entry.transfer_id || "Transfer matched"}</div>{entry.records_page_url && <a href={entry.records_page_url} target="_blank" rel="noreferrer" className="mt-1 inline-flex items-center gap-1 text-violet-700 hover:underline"><ExternalLink className="size-3" />Docterz records</a>}</> : entry.source_kind === "docterz_record" ? <><div className="font-semibold text-violet-700">Docterz Record</div>{entry.record_url && <a href={entry.record_url} target="_blank" rel="noreferrer" className="mt-1 inline-flex items-center gap-1 hover:underline"><ExternalLink className="size-3" />Open record</a>}</> : entry.source_kind === "razorpay" ? <><div className="font-mono">{entry.transfer_id || "—"}</div><div className="mt-1 capitalize text-muted-foreground">{entry.settlement_status}</div></> : <><div>{entry.invoice_no ? `Invoice #${entry.invoice_no}` : "Front Office activity"}</div><div className="mt-1 capitalize text-muted-foreground">{entry.payment_mode || entry.source_type}</div></>}</td>
                      <td className="p-3 text-right">{entry.source_kind === "consolidated" ? <>{entry.front_office_record_count ? <div className="font-bold">API collected {formatMoney(entry.net_amount)}</div> : <div className="font-medium text-violet-700">Docterz record linked</div>}<div className="mt-1 text-[10px] font-semibold text-emerald-700">Razorpay {formatMoney(entry.razorpay_net_amount || 0)}</div>{entry.pending_amount > 0 && <div className="mt-1 text-[10px] text-amber-600">Pending {formatMoney(entry.pending_amount)}</div>}{entry.reversed_amount > 0 && <div className="mt-1 text-[10px] text-rose-600">Reversed {formatMoney(entry.reversed_amount)}</div>}</> : entry.source_kind === "docterz_record" ? <span className="text-muted-foreground">Clinical record</span> : <><div className="font-bold">{formatMoney(entry.net_amount)}</div>{entry.source_kind === "front_office" && entry.gross_amount !== entry.net_amount && <div className="mt-1 text-[10px] text-muted-foreground">Billed {formatMoney(entry.gross_amount)}</div>}{entry.reversed_amount > 0 && <div className="mt-1 text-[10px] text-rose-600">Reversed {formatMoney(entry.reversed_amount)}</div>}{entry.pending_amount > 0 && <div className="mt-1 text-[10px] text-amber-600">Pending {formatMoney(entry.pending_amount)}</div>}</>}</td>
                      <td className="p-3"><Badge variant="outline" className={entry.source_kind === "consolidated" ? "border-emerald-300 bg-emerald-50 text-emerald-700" : "capitalize"}>{entry.source_kind === "consolidated" ? "Matched · Consolidated" : entry.source_kind === "docterz_record" ? "Docterz" : entry.source_kind === "front_office" ? entry.source_type : entry.reconciliation_status}</Badge><div className="mt-1 max-w-52 text-[10px] text-muted-foreground">{entry.source_kind === "consolidated" ? `${entry.api_record_count} API + ${entry.razorpay_record_count} Razorpay record(s)` : entry.file_name}</div></td>
                      <td className="p-3"><div className="max-w-44 truncate" title={entry.file_name}>{entry.file_name}</div><div className="mt-1 text-[10px] text-muted-foreground">{entry.imported_at ? new Date(entry.imported_at).toLocaleString("en-IN") : "—"}</div></td>
                    </tr>
                  ))}
                </tbody>
              </table>
